Public Class Galaxy 'Object that encompances the game
    ...
    Public Sub Client_Commands() 'Runs the commands from the Clients
        ...
        If ClientInteractions.ThrottleUp = True Then 'Increase the Client Ship's Throttle
            Combat.ClientShip.Engineering.Throttle.Current = Combat.ClientShip.Engineering.Throttle.Current + Combat.ClientShip.Acceleration 'Increase the throttle
        ...
        If ClientInteractions.ThrottleDown = True Then 'Decrease the Client Ship's Throttle
            Combat.ClientShip.Engineering.Throttle.Current = Combat.ClientShip.Engineering.Throttle.Current - Combat.ClientShip.Acceleration 'Decrease the throttle
        ...
        If ClientInteractions.ShipRight = True Then 'Turn the Ship to the right
            Combat.ClientShip.Direction = Server.Normalise_Direction(Combat.ClientShip.Direction + Combat.ClientShip.TurnSpeed) 'Turn the Ship
        End If
        ...
        Select Case State
            ...
            Case Star_Crew_Shared_Libraries.Shared_Values.GalaxyStates.Ship_To_Ship
                ...
                For i As Integer = 0 To Combat.Combatants.Count - 1 'Loop through all Ships
                    ...
                    positions(i) = New System.Drawing.Point(Combat.Combatants(i).X, Combat.Combatants(i).Y) 'Set this objects position
                    allegiances(i) = Combat.Combatants(i).ParentFleet.myAllegiance 'Set this objects allegiance to it's Allegiance
                Next
                '--------------------------
                '-----Send Message to Clients-----
                messageBuff = Game_Library.Serialisation.ToBytes(New Star_Crew_Shared_Libraries.Networking_Messages.ShipView(types, positions, allegiances)) 'Generate the message to send to the Clients
